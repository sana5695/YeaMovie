...
export class MoviesView {
    ...
    constructor(options) {
        const {className, movieController, data = [], filter, type} = options;
    ...
    update({movies, name}) {
        if (!observer.getState(name)) {
            observer.notify(name, movies);
            this.movieController.update(this.renderCard(movies));
        } else {
            this.movieController.update(this.renderCard(observer.getState(name)));
        }
    ...
    onLoadMovies = (data) => {
        ...
        if (observer.getState(name)){
            observer.notify(this.LoadMoviesKey, {name:name,movies:observer.getState(name)});
            return
        }
        if (url) this.controller.getMovies(url, name)
        else this.update()
    }
    ...
    cardListener(location) {
        location.addEventListener('click', event => {
            if (event.target.closest('.card')) {
                ...
                const movie = observer.getState(this.LoadMoviesKey).movies.filter(x => x.id.toString() === id)[0]
    ...
    render() {
        ...
        if (this.data.length > 1) {
            NavButtons.create({
                root: this.section,
                data: this.data,
                observerKey: this.GetMoviesKey
            });
        }

        this.movieController.mount(this.section, this.className);
        this.onLoadMovies(this?.data[0])
    }
}